...
""" scrape youtube channel to build table of contents html file and csv of video information for excel file
...
def channel_section_links():  # -> list of { 'title': <section title>, 'link': <url to section play lists> }
    ...
    if len(elements) == 0: # no sections, make up no sections section with default link
        elements = [ {'title':'no sections', 'link':f'https://youtube.com/{channel_name}/playlists'}]
...
def get_playlists(section):
    ...
    if playlists == []: return [{'title':'No Playlists', 'link':f'https://youtube.com/{channel_name}/videos'}]
    return playlists
...
def add_videos(playlist):
    ...
    for i in items:
        ...
        link = fix_url(i['href'])
        d['link'] = link
        t = i.find_next('span', { 'aria-label': True}) # find next span with attribute aria-label used for time
        ...
        d['publication_date'] = vsoup.find('strong', class_="watch-time-text").text[len('Published on ')-1:]
    ...
    for section in sections:
        ...
        for playlist in section['playlists']:
            ...
            for video in playlist['videos']:
                parts.append(tag('li',link(video['title'], video['short_link']) + ' (' + video['time'] + ")"))
            parts.append('</ol>')
...
def csv_out(channel, sections): # headers chanel name, playlist name, video name, short url, time,
    headers = 'channel,section,playlist,video,link,time,views,publication date,likes,dislikes,description'.split(',')
...
if __name__ == '__main__':
    ...
    print(f"Program Complete,\n{channel_name}.htm and {channel_name}.csv have been written to current directory")
```
